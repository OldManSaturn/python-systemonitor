...
def create_network_info_tab():
    network_info = psutil.net_if_addrs()
    network_info_column = ft.Column(
        [
            create_heading("Network Information")
        ],
        alignment=ft.MainAxisAlignment.CENTER,
        spacing=5
    )
    ...
    for interface, addresses in network_info.items():
        network_info_column.controls.append(create_text_component("Interface", interface))
        for addr in addresses:
            address_info = f"Address: {addr.address}, Netmask: {addr.netmask}, Broadcast: {addr.broadcast}"
            network_info_column.controls.append(ft.Text(address_info))

    network_info_container = ft.Container(
        content=network_info_column,
        padding=5,
        alignment=ft.alignment.center
    )

    return network_info_container
